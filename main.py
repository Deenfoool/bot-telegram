...
main_reply_menu = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="📊 Анализ системы")]
    ],
...
async def cmd_start(message: types.Message):
    welcome_text = (
        ...
        "📊 <b>Анализ системы:</b> Отправь файл отчета о системе, и я дам рекомендации по улучшению и устранению ошибок.\n\n"
        "<b>Важно:</b>\n"
...
@dp.message(lambda m: m.text == "📊 Анализ системы")
async def ask_for_system_report(message: types.Message):
    await message.answer(
        "✅ Отправьте файл <code>full_system_report.txt</code>, который вы создали с помощью скрипта, и я проанализирую его.\n\n"
        "Я постараюсь найти ошибки, предупреждения, подозрительные процессы, дать рекомендации по оптимизации и улучшению работы системы.",
        parse_mode="HTML"
    )

@dp.message(lambda m: m.document and m.document.mime_type == "text/plain")
async def handle_txt_file(message: types.Message):
    ...
    analysis = await analyze_system_report(content)
...
async def analyze_system_report(content: str) -> str:
    prompt = f"""
Ты — опытный системный администратор и специалист по Windows. Ниже приведён отчёт о системе Windows 11. Проанализируй его и найди:

1. Ошибки (Error, Critical, Fail)
2. Предупреждения (Warning)
3. Потенциальные проблемы (например, мало места на диске, подозрительные службы, драйверы, процессы)
4. Возможные рекомендации по оптимизации (службы, автозагрузка, диски, память и т.д.)

Выведи краткий отчёт на русском языке, разделив на 4 раздела: Ошибки, Предупреждения, Проблемы, Рекомендации.
...
"""
    ...
    try:
        ...
        return "❌ Не удалось получить анализ от OpenAI. Попробуйте позже."
